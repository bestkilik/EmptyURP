...
//HairCore
#ifndef UNIVERSAL_HAIRCORE_INCLUDED
#define UNIVERSAL_HAIRCORE_INCLUDED
...
//effect
uniform half _propertyF0_0;
uniform half _propertyF0_1;
uniform half4 _propertyC0;
uniform half _propertyF1_0;
uniform half _propertyF1_1;
uniform half4 _propertyC1;
uniform half _propertyF2_0;
uniform half4 _propertyC2;
uniform half _propertyF3_0;
uniform half4 _propertyC3;
//base prop
#ifdef NORMALTEX
	half _BumpScale;
#endif
...
TEXTURE2D(_HairTangentMap); SAMPLER(sampler_HairTangentMap);
#elif defined(HAIRTILINGFLOW)
TEXTURE2D(_HairTilingFlowmap); SAMPLER(sampler_HairTilingFlowmap);
#endif
...
half4 EffectColor(half4 nv)
{
    half4 finalColor;
    half4 dotResult18 = nv;
    half4 temp_output_46_0 = ((1.0 - max(0.0, dotResult18)) + 0.1);
    float4 blendOpSrc48 = (saturate(PositivePow(temp_output_46_0, _propertyF0_1)) * _propertyF0_0 * _propertyC1);
    float4 blendOpDest48 = (float4(0, 0, 0, 0) * _propertyF1_1 * _propertyC2 * saturate(
        PositivePow(temp_output_46_0, _propertyF1_1)));
    finalColor = ((saturate((1.0 - (1.0 - blendOpSrc48) * (1.0 - blendOpDest48)))) + (1.0
        * _propertyF2_0 * _propertyC3) + (float4(0, 0, 0, 0) * _propertyC3 * _propertyF3_0));
...
{
    VertexOutputBase o = (VertexOutputBase)0;

    VertexPositionInputs vertexInput = GetVertexPositionInputs(v.vertex.xyz);
    VertexNormalInputs normalInput = GetVertexNormalInputs(v.normal, v.tangent);
    float3 posWorld = vertexInput.positionWS;
    ...
    float3 tangentWorld = normalInput.tangentWS;
    ...
    o.shadowCoord = TransformWorldToShadowCoord(posWorld);
    #if defined (GI)
...
half4 fragBase(VertexOutputBase i, half facing : VFACE) : SV_Target
{
    ...
    half3 posWorld = half3(i.tangentToWorldAndPackedData[0].w, i.tangentToWorldAndPackedData[1].w,
    ...
    half4 albedoColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, texcoord);
    ...
    half3 specColor = lerp(kDielectricSpec.rgb, albedo.rgb, metallic);
    ...
    #if defined (HAIRTANGENTMAP)
    half4 hairTangentmap = SAMPLE_TEXTURE2D(_HairTangentMap,sampler_HairTangentMap, texcoord);
    ...
    Light light = GetMainLight(i.shadowCoord, posWorld, shadowMask);
    half atten = light.distanceAttenuation * light.shadowAttenuation;
    half3 mainLight = light.color.rgb * saturate(atten);
	...
	half3 normalTex = UnpackNormalScale(tex2D(_BumpMap, texcoord), _BumpScale);
    ...
    GIspecular = SAMPLE_TEXTURE2D_LOD(unity_SpecCube0, samplerunity_SpecCube0, reflUVW.xyz, mip).rgb * occ;
    GIspecular = DecodeHDREnvironment(GIspecular.rgbr, unity_SpecCube0_HDR);
    GIdiffuse = occ * i.vertexSH.rgb;
    ...
    #ifdef AUTOSPEC
		half3 specHSVcolor1 = RGBtoHSV(light.color.rgb);
		specHSVcolor1.x = saturate(specHSVcolor1.x > 0.5? specHSVcolor1.x + 0.2 : specHSVcolor1.x - 0.2 );
    ...
    #ifdef DOUBLELOBE
			half3 specHSVcolor2 = RGBtoHSV(light.color.rgb);
			specHSVcolor2.x = saturate(specHSVcolor2.x > 0.5? specHSVcolor2.x - 0.5 : specHSVcolor2.x + 0.5 );
    ...
    half3 spec1 = KajiyaSpecular(t1, viewDir, lightDir.xyz, _SpecPower1 * 128.0) * _SpecColor1.rgb * _SpecularAmount1;
    specularColor += saturate(spec1);
    #ifdef DOUBLELOBE
		half3 spec2 =KajiyaSpecular(t2, viewDir, lightDir.xyz, _SpecPower2 * 128.0)*_SpecColor2.rgb * _SpecularAmount2;// to do: spec2 need spec mask
...
half4 fragBasePreZ(VertexOutputBase i, half facing : VFACE) : SV_Target
{
