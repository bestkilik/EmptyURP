Shader "GE/Scene/Basic_Grass"
{
	Properties
	{
		...
		[ASEEnd]_NormalScale("Normal Scale", Range( 0 , 2)) = 1

		[Space(20)][Header(Grass WPO)]
		_WPOWaveSpeed("WPO Wave Speed", Float) = 0.14
		_WPOWaveAmount("WPO Wave Amount", Float) = 0.1
	...
	SubShader
	{
		...
		{
			half atten = (light.shadowAttenuation * 0.5 + 0.5) * light.distanceAttenuation;
			return LambertLitBased(albedo, light.color, light.direction, atten, normalWS, viewDirectionWS);
		}
		...
		Pass
		{
			...
			VertexOutput vert( VertexInput v  )
			{
				...
				#ifndef LowQuality
				float3 rotAxis = normalize(cross(_WindVector.xyz, float3(0,1,0)));
				float3 worldPos = TransformObjectToWorld(v.vertex.xyz);
				float2 hPosWeight = float2(dot(_WindVector.xyz, worldPos), dot(rotAxis, worldPos))*0.5;
				float2 spnT = (float2(1, 2) * _WPOWaveSpeed) * _FixTime.y + hPosWeight;
				spnT = sin(spnT);
				spnT = (0.6*spnT+0.4) * float2(-0.1, -0.05);
				float rotAngle = spnT.x + spnT.y;
				float3 objPos = mul(GetObjectToWorldMatrix(), float4(0,0,0,1)).xyz;
				float3 heightWeight = worldPos.y - objPos.y;
				heightWeight = heightWeight * float3(1, -0.5, 1) * _WPOWaveAmount;
				
				float3 wpo = RotateAroundAxis(worldPos + float3(0,-1,0), worldPos, rotAxis, rotAngle);
				wpo *= heightWeight;

				float3 positionWS = TransformObjectToWorld( v.vertex.xyz ) + wpo;
				#else
				...
				float3 wno = float3(0,0,0);
				#ifndef LowQuality
				wno = RotateAroundAxis(float3(0,0,0), normalInput.normalWS, rotAxis, rotAngle);
				wno *= heightWeight;
				#endif

				o.tSpace0 = float4( normalInput.normalWS, positionWS.x);
				o.tSpace1 = float4( normalize(normalInput.tangentWS + wno), positionWS.y);
