Shader "GE/Scene/Basic_CustomLightmap"
{
	Properties
	{
		[Enum(UnityEngine.Rendering.CullMode)]_Cull("Cull", Float) = 2
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[NoScaleOffset][Header(Main Properties)]_LightMap("Light Map (uv2)", 2D) = "black" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv39 = uv_MainTex;
				float4 temp_output_7_0 = ( _Color * tex2D( _MainTex, uv39 ) );
				float3 albedo14 = (temp_output_7_0).rgb;
				
				float3 unpack12 = UnpackNormalScale( tex2D( _BumpMap, uv39 ), _NormalScale );
				...
				float3 emissive34 = ( ( (tex2D( _LightMap, texCoord41 )).rgb * 2.0 ) * albedo14 );
				
				float alpha15 = (temp_output_7_0).a;
				
				float3 Albedo = albedo14;
				float3 Normal = normal16;
				float3 Emission = emissive34;
				float Occlusion = 1;
				float Alpha = alpha15;
