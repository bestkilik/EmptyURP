Shader "GE/Scene/Water_WaterfallFlow"
{
	Properties
	{
		...
		[NoScaleOffset] _WaterSetsTex("水參數 R:Foam G:ShoreNoise B:Splash", 2D) = "black" {}
		_SplashColor ("Splash Color", Color) = (0.952,0.952,0.952,1)
	...
	SubShader
	{
		Tags { "RenderPipeline"="UniversalPipeline" "IgnoreProjector" = "True" "Queue" = "Transparent+20" "RenderType" = "Transparent" }
		LOD 200
		ZWrite Off
		...
		pass
		{
			...
			half4 frag(v2f i): SV_Target
			{
				...
				float2 uvSlpash = flowUV*float2(1.0,4.0);
				...
				// half texForm =  lerp(tex2D(_WaterSetsTex, flowUV0*0.75 + uvForm).r, tex2D(_WaterSetsTex, flowUV1*0.75 + uvForm + float2(0.15,0.15)).r, flowMask);
				// half formMask = texForm * (1 - i.color.b);
				half splash = lerp(tex2D(_WaterSetsTex, flowUV0 + flowUV).b, tex2D(_WaterSetsTex, flowUV1 + flowUV + float2(0.15,0.15)).b, flowMask);
				// splash*=splash;
				...
				half3 waterColor = _ColorWater;//lerp(_ColorWater, _ColorDeep ,deepDepth);
				...
				outColor.rgb = lerp(sceneColor, difLit * _SplashColor.rgb, splash);
