Shader "GE/Scene/Standard_PP-Rotate"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput vert( VertexInput v  )
			{
				...
				float3 pivPos = float3(v.texcoord2.xy, v.texcoord3.x);
				float3 rotAxis = v.color.rgb*2.0-1.0;
				float ppMask = dot(v.color.rgb, float3(1,1,1));
				ppMask = ppMask > 0.0 ? 1.0 : 0.0;

				float rotProc = _RotateSpeed * _FixTime.y;
				#ifdef _COVERSNOW
					...
					rotProc = snowMask > 0.1 ? 0.0 : rotProc;
				#endif

				float3 aniPos = RotateAroundAxis(pivPos, v.vertex.xyz, rotAxis, rotProc);
				v.vertex.xyz = lerp(v.vertex.xyz, aniPos, ppMask);
				
				float3 rotNrm = RotateAroundAxis(float3(0,0,0), v.normal, rotAxis, rotProc);
				v.normal = lerp(v.normal, rotNrm, ppMask);
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float occlusion32 = tex2D(_PhysicMap, IN.ase_texcoord7.zw).b;
