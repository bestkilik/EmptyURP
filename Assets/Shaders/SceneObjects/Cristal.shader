Shader "GE/Scene/Ctristal"
{
	Properties
	{
		...
		[HDR]_RingGlow("Ring Glow", Color) = (0,0,0,1)
		[Header(Refract And Rraflect)]
		_IOR ("IOR", Range(1.0, 3)) = 1.4
		_FilterDiffusionLevel("Filter Diffusion Level", Range(0, 0.75)) = 0.1
	...
	SubShader
	{
		...
		half U_D_GGX(CristalBRDFData brdfData, half HdN, half LdH2)
		{
		...
		inline void AdvInitializeBRDFData(half3 albedo, half metallic, half specular, half smoothness, half alpha, half3 ringGlow, out CristalBRDFData outBRDFData)
		{
			...
			outBRDFData.filterColor = albedo / max(max(max(albedo.r, albedo.g), albedo.b),0.0001);
			outBRDFData.diffuse = albedo * oneMinusReflectivity;
			outBRDFData.specular = lerp(/* kDieletricSpec.rgb */0.1, albedo, metallic) * speculerLevel;
			outBRDFData.metallic = metallic;
			outBRDFData.grazingTerm = saturate(smoothness + reflectivity) * speculerLevel * 4;
			...
			outBRDFData.glowColor = ringGlow;
		...
		half3 CristalLitBased(CristalBRDFData brdfData, Light light, half3 normalWS, half3 viewDirectionWS)
		{
			...
			half3 DiffLit = light.color * (atten * LdN);
			
			float3 H = SafeNormalize(L + N);
		...
		half3 CristalGlobalIllumination(CristalInputData inputData, CristalBRDFData brdfData, half occlusion)
		{
			...
			float3 viewN = mul(UNITY_MATRIX_V, float4(inputData.normalWS, 0.0)).xyz;
			// float3 scRefraDir = viewN * (eta - 1.0);
			float3 scRefraDir = (viewN - viewN*inputData.eta.x); // viewN*inputData.eta.x - viewN;
			inputData.normalizedScreenSpaceUV = inputData.normalizedScreenSpaceUV + scRefraDir.xy;
			// inputData.normalizedScreenSpaceUV = inputData.normalizedScreenSpaceUV - floor(inputData.normalizedScreenSpaceUV);
			half3 ScnColor = SAMPLE_TEXTURE2D_X(_CameraOpaqueTexture, sampler_CameraOpaqueTexture, inputData.normalizedScreenSpaceUV).rgb;
			ScnColor = (ScnColor - ScnColor*inputData.eta.y) + inputData.eta.y;
			c += ScnColor * brdfData.filterColor;
			c += brdfData.glowColor * fresnelTerm;
			// c = brdfData.specular;
			// c = SHADERGRAPH_SAMPLE_SCENE_COLOR(inputData.screenPosNrm).rgb;
			return c;
		}

		half4 UniversalCristalPBR(CristalInputData inputData, half3 albedo, half metallic, half specular, half smoothness, half occlusion, half3 emission, half3 ringGlow, half alpha)
		{
			CristalBRDFData brdfData;
			AdvInitializeBRDFData(albedo, metallic, specular, smoothness, alpha, ringGlow, brdfData);
			...
			half3 color = CristalGlobalIllumination(inputData, brdfData, occlusion);
			color += CristalLitBased(brdfData, mainLight, inputData.normalWS, inputData.viewDirectionWS);
				...
				for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
				{
					...
					color += CristalLitBased(brdfData, light, inputData.normalWS, inputData.viewDirectionWS);
				}
			#endif
		...
		Pass
		{
			...
			#define ASE_NEEDS_FRAG_SCREEN_POSITION 1
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				CristalInputData inputData;
				inputData.eta = half2(1.0/_IOR, _FilterDiffusionLevel);
				...
				Albedo = lerp(lerp(desatAlbedo * 0.75, Albedo, (1 + Metallic) - Metallic*coverMask), snowColor, snowMask * (topCoverMask + frostMask));
				...
				half4 color = UniversalCristalPBR(
					inputData, 
					Albedo, 
					Metallic, 
					Specular, 
